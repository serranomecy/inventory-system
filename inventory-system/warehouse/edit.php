...
$id=intval($_GET['id']);
$result=$conn->query("SELECT * FROM warehouse WHERE Warehouse_ID=$id");
...
if(isset($_POST['submit'])){
    ...
    if(empty($name) || empty($location)){ $error="All fields are required!"; }
    else{
        $sql="UPDATE warehouse SET Warehouse_Name='$name', Location='$location' WHERE Warehouse_ID=$id";
...
<input type="text" name="name" value="<?= $row['Warehouse_Name']?>" required>
